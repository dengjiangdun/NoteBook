package com.johndon.cmcc.zhuomian.util;
...
public class TimeUtils {
    ...
    public static String secondToTime(String releasseDate) {
        ...
        String data = format.format(db.longValue() );      //根据时间戳毫秒获取格式为yyyy-MM-dd的时间

        long between = (now.getTime()- db.longValue() )/1000;  //8*60*60这个是为了处理时区问题
        ...
        if (day > 14) {
            ...
            result = String.valueOf(day) + "days ago";
        } else if (hour > 0) {
            result = String.valueOf(hour) + "hours ago";
        } else if (minute > 0) {
            result = String.valueOf(minute) + "minutes ago";
        } else if (second > 0){
            result = String.valueOf(second) + "seconds ago";
        }else{
            result = "just now";
        }
        return result;
