package com.example.gionee.notebook.view;
...
public class RevealLayout extends LinearLayout implements Runnable{
    ...
    private void initParametersForChild(MotionEvent event,View view){
        ...
        mIsPressed = true;
        mRevealRadiuGap = mMinBetweenWidthAndHeight/8;
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (mRevealRadius > mMinBetweenWidthAndHeight / 2) {
            mRevealRadius += mRevealRadiuGap * 4;
        } else {
            mRevealRadius += mRevealRadiuGap;
        }
        this.getLocationOnScreen(mLocationInScreen);
        int[] location = new int[2];
        ...
        int bottom = top + mTouchTarget.getPaddingRight();
    ...
    private View getTouchTarget(View view,int x,int y){
        ...
        for (View child: TouchableViews){
            if (isTouchPoiintInView(child,x,y)){
                target = child;
                break;
            }
        }
    ...
    private boolean isTouchPoiintInView(View view,int x,int y){
        int[] location = new int[2];
        view.getLocationOnScreen(location);
        int left = location[0];
        int top = location[1];
        ...
        if (view.isClickable() && y<=bottom && top <= y && x<= right && left <= x){
            return true;
        }
        return false;
    }
    ...
    private class DispatchUpTouchEventRunnable implements Runnable{
        ...
        public void run() {
            ...
            if (isTouchPoiintInView(mTouchTarget,(int) event.getRawX(),(int) event.getRawY()));{
